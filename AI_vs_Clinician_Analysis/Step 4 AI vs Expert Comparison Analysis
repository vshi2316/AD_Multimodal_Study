...
if (!"AI_Probability" %in% colnames(ai_pred) && "Predicted_Prob" %in% colnames(ai_pred)) {
  ai_pred <- ai_pred %>% rename(AI_Probability = Predicted_Prob)
}
if (!"CaseID" %in% colnames(ai_pred)) {
  for (candidate in c("ID", "PTID", "RID", "Subject")) {
    if (candidate %in% colnames(ai_pred)) {
      ai_pred <- ai_pred %>% rename(CaseID = !!candidate)
      break
    }
  }
}
