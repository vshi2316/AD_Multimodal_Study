...
train_file = "./cluster_results.csv"

if not os.path.exists(input_file):
    print(f"ERROR: Test set file not found - {input_file}")
    exit(1)

if not os.path.exists(train_file):
    print(f"ERROR: Training set file not found - {train_file}")
    exit(1)
...
print("\n[PHASE 4] MICE Imputation (Frozen Pipeline)")
print("-" * 70)
...
print("\n[PHASE 5] Standardization (Frozen Pipeline)")
print("-" * 70)
...
print("\n[PHASE 6] KernelPCA (Frozen Pipeline)")
print("-" * 70)
...
X_train_latent_df = pd.DataFrame(X_train_latent, columns=[f'Latent_{i+1}' for i in range(5)])
X_test_latent_df = pd.DataFrame(X_test_latent, columns=[f'Latent_{i+1}' for i in range(5)], index=X_test_imputed.index)

X_train_full = pd.concat([X_train_imputed.reset_index(drop=True), X_train_latent_df], axis=1)
X_test_full = pd.concat([X_test_imputed.reset_index(drop=True), X_test_latent_df.reset_index(drop=True)], axis=1)

print("\n[PHASE 7] Lasso Feature Selection (Frozen Pipeline)")
print("-" * 70)
...
print("\n[PHASE 8] Model Training (Training Set Only)")
print("-" * 70)
...
clf_gbm = HistGradientBoostingClassifier(learning_rate=0.05, max_iter=500, max_depth=8,
...
print("\n[PHASE 9] Model Evaluation on Independent Test Set")
print("-" * 70)

print("Training SVM-RBF...")
pipeline_svm = create_pipeline(clf_svm)
pipeline_svm.fit(X_train_selected, y_train)
methods_preds['SVM-RBF'] = pipeline_svm.predict_proba(X_test_selected)[:, 1]

print("Training Gradient Boosting...")
pipeline_gbm = create_pipeline(clf_gbm)
...
print(f"Risk thresholds: High >= {HIGH_THRESHOLD}, Medium >= {LOW_THRESHOLD}")
...
report_lines = [
    ...
    "  All preprocessing parameters fitted on training set only",
    "  - MICE imputation: fitted on training set",
    "  - StandardScaler: fitted on training set",
    "  - KernelPCA: fitted on training set",
    "  - Lasso selector: fitted on training set",
    "  - All models: trained on training set",
    "  Test set used ONLY for final independent evaluation",
    "",
    "1. Core Methodology",
    "   - Feature Selection: Lasso (L1) regularization",
    "   - Modeling: SVM-RBF + GBM + RF ensemble",
    "   - Latent Space: KernelPCA",
    "   - Imputation: MICE iterative imputation",
    "",
    "2. Test Set Characteristics",
    f"   Sample size: {len(df_test)} cases",
    f"   AD converters: {df_test['AD_Conversion'].sum()} ({df_test['AD_Conversion'].mean():.1%})",
    f"   Non-converters: {(df_test['AD_Conversion']==0).sum()} ({(df_test['AD_Conversion']==0).mean():.1%})",
    "",
    "3. Model Performance on Independent Test Set"
]

for model_name in ['SVM-RBF', 'Gradient Boosting', 'Random Forest', 'Ensemble (Final)']:
    auc_val = model_aucs[model_name]
    report_lines.append(f"   {model_name}: AUC={auc_val:.3f}")
