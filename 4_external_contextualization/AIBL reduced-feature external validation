...
adni_path <- file.path(base_dir, "Phase1_ADNI_Discovery", "ADNI_Labeled_For_Classifier.csv")
adni_demog_path <- file.path(base_dir, "ADNI原始数据", "LINES", "Subject Demographics.csv")
adni_mmse_path <- file.path(base_dir, "ADNI原始数据", "LINES", "Mini-Mental State Examination (MMSE).csv")
adni_cdr_path <- file.path(base_dir, "ADNI原始数据", "LINES", "Clinical Dementia Rating.csv")
adni_apoe_path <- file.path(base_dir, "ADNI原始数据", "APOE", "ApoE Genotyping - Results.csv")
aibl_raw_dir <- file.path(base_dir, "aibl_19Sep2019", "Data_extract_3.3.0")
aibl_gate_dir <- file.path(base_dir, "AIBL验证", "AIBL_MRI_Download_Targets")
