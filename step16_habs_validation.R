...
option_list <- list(
  ...
  make_option(c("--ptau_col"), type = "character", default = "pTau217_Primary",
              help = "p-tau217 column name [default: %default]")
)
...
# Check p-tau217 column
ptau217_col <- opt$ptau_col
if (!ptau217_col %in% colnames(habs_data)) {
  # Try alternative names
  alt_names <- c("pTau217", "ptau217", "pTau217_Primary", "PTAU217")
  for (alt in alt_names) {
    if (alt %in% colnames(habs_data)) {
      ptau217_col <- alt
      break
    }
  }
  if (!ptau217_col %in% colnames(habs_data)) {
    stop(sprintf("p-tau217 column not found. Tried: %s", 
                 paste(c(opt$ptau_col, alt_names), collapse = ", ")))
  }
}
...
train_idx <- sample(seq_len(nrow(habs_complete)),
                    size = floor(opt$train_ratio * nrow(habs_complete)))
...
cat(sprintf("\nBaseline model AUC: %.3f (95%% CI %.3f–%.3f)\n",
...
habs_test <- habs_test %>%
  mutate(
    ...
    Low_Cognition   = ifelse(MMSE_Baseline < median(MMSE_Baseline, na.rm = TRUE), 1, 0)
  )
...
# Calculate AUC for all subgroups
...
cat(sprintf("Complete model AUPRC is %.2fx random\n", fold_improvement))
...
for (i in seq_len(opt$n_bootstrap)) {
...
idi_ci      <- as.numeric(quantile(idi_bootstrap, c(0.025, 0.975), na.rm = TRUE))
idi_p_value <- 2 * min(mean(idi_bootstrap > 0, na.rm = TRUE), 
                       mean(idi_bootstrap < 0, na.rm = TRUE))
...
# ==============================================================================
# Part 3: Decision Curve Analysis and Visualizations
# ==============================================================================
...
thresholds_dca <- seq(0.01, 0.8, by = 0.01)
...
ggsave(file.path(opt$output_dir, "step16_fig1_roc_comparison.pdf"), p1,
       width = 8, height = 6, dpi = 300)
cat("Saved: step16_fig1_roc_comparison.pdf\n")

# Figure 2: Subgroup AUC Forest Plot
...
ggsave(file.path(opt$output_dir, "step16_fig2_subgroup_auc_forest.pdf"), p2,
       width = 10, height = 6, dpi = 300)
cat("Saved: step16_fig2_subgroup_auc_forest.pdf\n")
...
ggsave(file.path(opt$output_dir, "step16_fig3_precision_recall.pdf"), p3,
       width = 8, height = 6, dpi = 300)
cat("Saved: step16_fig3_precision_recall.pdf\n")
...
p4 <- ggplot(dca_data, aes(x = Threshold, y = Net_Benefit, color = Strategy)) +
  ...
  annotate("text", x = optimal_threshold + 0.05, y = max_net_benefit,
           label = sprintf("Optimal: %.3f", optimal_threshold),
...
ggsave(file.path(opt$output_dir, "step16_fig4_decision_curve.pdf"), p4,
       width = 10, height = 6, dpi = 300)
cat("Saved: step16_fig4_decision_curve.pdf\n")

# Figure 5: NRI and IDI
nri_data <- data.frame(
...
ggsave(file.path(opt$output_dir, "step16_fig5_nri_idi.pdf"), p5,
       width = 8, height = 6, dpi = 300)
cat("Saved: step16_fig5_nri_idi.pdf\n")
...
ggsave(file.path(opt$output_dir, "step16_fig6_combined_analysis.pdf"), p_combined,
       width = 16, height = 12, dpi = 300)
cat("Saved: step16_fig6_combined_analysis.pdf\n")

cat("\nPart 3 complete.\n\n")

# ==============================================================================
# Part 4: Save Results and Summary Report
# ==============================================================================
cat("Part 4: Saving results and generating summary report\n")
cat("------------------------------------------------------------\n")
...
save(habs_data, habs_complete, habs_train, habs_test,
     ...
     auc_subgroups,
     auprc_base, auprc_complete, baseline_auprc, fold_improvement,
     nri_results, nri_result,
     idi_value, idi_ci, idi_p_value, idi_bootstrap,
     dca_results,
     file = file.path(opt$output_dir, "step16_habs_all_results.RData"))

cat("Saved: step16_habs_all_results.RData\n")

# Save summary tables
write.csv(auc_subgroups, 
          file.path(opt$output_dir, "step16_subgroup_auc.csv"), 
          row.names = FALSE)
write.csv(nri_data, 
          file.path(opt$output_dir, "step16_nri_idi_summary.csv"), 
          row.names = FALSE)
...
summary_lines <- c(
  ...
  "Data Summary",
  "--------------------------------------------------------------------------------",
  sprintf("  Total HABS sample: %d", nrow(habs_data)),
  sprintf("  p-tau217 available: %.1f%%", ptau_rate),
  sprintf("  Complete cases: %d (%.1f%%)", nrow(habs_complete), 100 * complete_rate),
  sprintf("  Training set: %d (events=%d)", nrow(habs_train), sum(habs_train$AD_Conversion)),
  sprintf("  Test set: %d (events=%d)", nrow(habs_test), sum(habs_test$AD_Conversion)),
  "",
  "--------------------------------------------------------------------------------",
  "Model Performance",
  "--------------------------------------------------------------------------------",
  sprintf("  Baseline model AUC: %.3f (95%% CI %.3f–%.3f)", 
          auc_base, ci_base[1], ci_base[3]),
  sprintf("  Complete model AUC: %.3f (95%% CI %.3f–%.3f)", 
          auc_complete, ci_complete[1], ci_complete[3]),
  sprintf("  Delta AUC: %.3f", auc_complete - auc_base),
  sprintf("  DeLong p-value: %.4e", roc_test$p.value),
  "",
  sprintf("  Baseline model AUPRC: %.3f", auprc_base$AUPRC),
  sprintf("  Complete model AUPRC: %.3f (%.2fx random)", 
          auprc_complete$AUPRC, fold_improvement),
  "",
  "--------------------------------------------------------------------------------",
  "Reclassification Metrics",
  "--------------------------------------------------------------------------------",
  sprintf("  NRI (High_Event_Rate): %.3f (%.1f%%)", 
          nri_result$total_nri, 100 * nri_result$total_nri),
  sprintf("  IDI: %.4f (95%% CI %.4f–%.4f, p=%.4f)", 
          idi_value, idi_ci[1], idi_ci[2], idi_p_value),
  "",
  "--------------------------------------------------------------------------------",
  "Decision Curve Analysis",
  "--------------------------------------------------------------------------------",
  sprintf("  Optimal threshold: %.3f", optimal_threshold),
  sprintf("  Maximum net benefit: %.4f", max_net_benefit),
  "",
  "--------------------------------------------------------------------------------",
  "Subgroup Analysis",
  "--------------------------------------------------------------------------------"
...
for (i in 1:nrow(auc_subgroups)) {
  summary_lines <- c(summary_lines,
    sprintf("  %s: N=%d, Events=%d (%.1f%%), AUC=%.3f",
            auc_subgroups$Subgroup[i],
            auc_subgroups$N[i],
            auc_subgroups$Events[i],
            auc_subgroups$Event_Rate[i],
            auc_subgroups$AUC[i])
  )
}
...
summary_lines <- c(summary_lines,
  ...
  "Output Files",
  "--------------------------------------------------------------------------------",
  sprintf("  - %s/step16_habs_all_results.RData", opt$output_dir),
  sprintf("  - %s/step16_subgroup_auc.csv", opt$output_dir),
  sprintf("  - %s/step16_nri_idi_summary.csv", opt$output_dir),
  sprintf("  - %s/step16_fig1_roc_comparison.pdf", opt$output_dir),
  sprintf("  - %s/step16_fig2_subgroup_auc_forest.pdf", opt$output_dir),
  sprintf("  - %s/step16_fig3_precision_recall.pdf", opt$output_dir),
  sprintf("  - %s/step16_fig4_decision_curve.pdf", opt$output_dir),
  sprintf("  - %s/step16_fig5_nri_idi.pdf", opt$output_dir),
  sprintf("  - %s/step16_fig6_combined_analysis.pdf", opt$output_dir),
  "",
  "================================================================================",
  "HABS External Validation Complete",
  "================================================================================"
)

# Write report
report_path <- file.path(opt$output_dir, "HABS_Validation_Report.txt")
...
cat("============================================================\n")
cat("Step 16: HABS External Validation Complete!\n")
cat("============================================================\n")
