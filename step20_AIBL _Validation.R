...
option_list <- list(
  make_option(c("--projection_file"), type = "character", default = "AIBL_projected_subtypes.csv",
              help = "Projection output from step9A [default: %default]"),
  make_option(c("--baseline_file"), type = "character", default = "AIBL_Baseline_Integrated.csv",
              help = "AIBL baseline file with outcomes [default: %default]"),
  make_option(c("--output_dir"), type = "character", default = "./step20_results",
...
write.csv(analysis, file.path(opt$output_dir, "step20_aibl_projected_subtypes.csv"), row.names = FALSE)
...
if (all(c(opt$time_col, opt$event_col) %in% names(analysis))) {
  ...
  write.csv(cox_out, file.path(opt$output_dir, "step20_aibl_cox.csv"), row.names = FALSE)
  ...
  write.csv(auc_out, file.path(opt$output_dir, "step20_aibl_auc.csv"), row.names = FALSE)
  ...
  write.csv(summary_out, file.path(opt$output_dir, "step20_aibl_summary.csv"), row.names = FALSE)

  pdf(file.path(opt$output_dir, "step20_aibl_kaplan_meier.pdf"), width = 7, height = 6)
  print(ggsurvplot(km_fit, data = analysis, risk.table = TRUE, pval = TRUE))
  dev.off()
}

cat("Saved Step 20 outputs\n")
